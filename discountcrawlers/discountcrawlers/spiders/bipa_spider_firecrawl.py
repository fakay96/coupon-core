"""
BIPA Spider using Firecrawl
=============================

This spider scrapes discount products from BIPA using Firecrawl API
...
class BipaFirecrawlSpider(BaseFirecrawlSpider):
    """BIPA spider using Firecrawl for enhanced scraping."""
    
    name = "bipa_spider_firecrawl"
    allowed_domains = ['www.bipa.at']
    start_urls = ['https://www.bipa.at/cp/aktionen']
    
    # Store-specific configuration
    store_config = get_store_config('bipa')
    max_pages = store_config.get('max_pages', 10)
    items_per_page = store_config.get('items_per_page', 50)
    
    # Custom settings for BIPA
    custom_settings = {
        **BaseFirecrawlSpider.custom_settings,
        'DOWNLOAD_DELAY': 1.5,  # BIPA rate limiting
    ...
    def _get_wait_actions(self) -> list:
        """Get wait actions specific to BIPA."""
        return [
            {
                "type": "wait",
    ...
    async def _extract_items_from_firecrawl_result(
        ...
        """Extract discount items from Firecrawl result with BIPA-specific logic."""
        items = []
        ...
        try:
            ...
            if 'data' in result and isinstance(result['data'], dict):
                ...
                # Handle BIPA's product structure
                if 'products' in structured_data:
                    # Multiple products in array
                    for product_data in structured_data['products']:
                        item = self._create_bipa_item_from_data(product_data, source_url, store_name)
                        if item:
                            items.append(item)
                
                elif 'product' in structured_data:
                    # Single product
                    item = self._create_bipa_item_from_data(structured_data['product'], source_url, store_name)
                    if item:
                        items.append(item)
                
                else:
                    # Try to extract from general data structure
                    item = self._create_bipa_item_from_data(structured_data, source_url, store_name)
                    if item:
                        items.append(item)
    ...
    def _create_bipa_item_from_data(
        self, 
        data: Dict[str, Any], 
        source_url: str, 
        store_name: str
    ) -> Optional[DiscountItem]:
        """Create a DiscountItem from BIPA data with specific field mapping."""
        try:
            item = DiscountItem()
            ...
            if not item.get('discount_percentage') and item.get('original_price') and item.get('sale_price'):
                try:
                    original = self._parse_bipa_price(item['original_price'])
                    sale = self._parse_bipa_price(item['sale_price'])
            ...
            item = self._clean_bipa_item(item)
            
            return item if self._validate_bipa_item(item) else None
            
        except Exception as e:
            self.logger.error(f"Error creating BIPA item from data: {e}")
            return None
    
    def _parse_bipa_price(self, price_str: str) -> Optional[float]:
        """Parse BIPA price format."""
        if not price_str:
            return None
    ...
    def _clean_bipa_item(self, item: DiscountItem) -> DiscountItem:
        """Clean and normalize BIPA item data."""
        # Clean price fields
        ...
        if item.get('url') and not item['url'].startswith('http'):
            if item['url'].startswith('/'):
                item['url'] = f"https://www.bipa.at{item['url']}"
            else:
                item['url'] = f"https://www.bipa.at/{item['url']}"
        
        return item
    
    def _validate_bipa_item(self, item: DiscountItem) -> bool:
        """Validate if a BIPA item has required fields."""
        # BIPA items should have at least a name or sale price
        return bool(
            item.get('name') or 
    ...
    def _get_next_page_url(self, current_url: str, next_page_num: int) -> Optional[str]:
        """Generate next page URL for BIPA pagination."""
        try:
