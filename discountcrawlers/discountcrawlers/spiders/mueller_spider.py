...
import re
import logging
from typing import Iterator, Dict, Any, Optional, List, AsyncIterator, ClassVar
...
class MuellerSpider(BaseDealsSpider):
    """
    Spider for scraping Mueller Austria outlet pages using Scrapy and Playwright.
    Extracts product details from dynamically rendered pages, handles pagination,
    and captures errors via screenshots.
    """
    name: ClassVar[str] = "mueller"
    allowed_domains: ClassVar[list[str]] = ["www.mueller.at"]
    start_urls: ClassVar[list[str]] = ["https://www.mueller.at/outlet"]
    ...
    # --- Selectors ---
    card_selector: ClassVar[str] = 'div.product-item'
    field_selectors: ClassVar[Dict[str, str]] = {
        "url": "a.product-item__link::attr(href)",
        "brand": "span.product-item__brand::text",
        "name": "span.product-item__name::text",
        "sale_price": "span.product-item__price--sale::text",
        "original_price": "span.product-item__price--original::text",
        "discount_percentage": "span.product-item__discount::text",
    }
    next_page_selector: ClassVar[str] = 'a.pagination__next:not(.disabled)'
    ...
    async def parse(self, response: Response) -> AsyncIterator[DiscountItem]:
        ...
        try:
            ...
            await this._handle_popups(page)
            
            # Wait for product cards
            await this._wait_for_cards(page)
            ...
            if not cards and current_page == 1:
                ...
                await this._save_screenshot_and_close(page, f"mueller_no_cards_page_{current_page}.png")
                return
            ...
            for card in cards:
                try:
                    ...
                    for field, selector in self.field_selectors.items():
                        ...
                        item[field] = this._clean(raw) if raw else None
            ...
            if current_page < this.MAX_PAGES:
                next_btn = page.locator(this.next_page_selector).first
                if await next_btn.count() and await next_btn.is_enabled(timeout=5000):
                    await next_btn.click(timeout=15000)
                    await page.wait_for_selector(this.card_selector, state="visible", timeout=30000)
            ...
            await this._save_screenshot_and_close(page, f"mueller_error_page_{current_page}.png")
        finally:
    ...
    async def _wait_for_cards(self, page: PlaywrightPage):
        ...
        try:
            await page.locator(this.card_selector).first.wait_for(
                state='visible',
                timeout=30000
            )
    ...
    async def _handle_popups(self, page: PlaywrightPage):
        ...
        try:
            cookie_button = page.locator(this.cookie_accept_selector).first
    ...
    async def errback_handle_error(self, failure):
        ...
        if page and not page.is_closed():
            await this._save_screenshot_and_close(page, f"mueller_errback_page_{page_num}.png")