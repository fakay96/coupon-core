"""
MUELLER Spider using Firecrawl
================================

This spider scrapes discount products from MUELLER using Firecrawl API
...
class MuellerFirecrawlSpider(BaseFirecrawlSpider):
    """MUELLER spider using Firecrawl for enhanced scraping."""
    
    name = "mueller_spider_firecrawl"
    allowed_domains = ['www.mueller.at']
    start_urls = ['https://www.mueller.at/c/parfuemerie/aktionen/?page=1', 'https://www.mueller.at/c/drogerie/aktionen/?page=1']
    
    # Store-specific configuration
    store_config = get_store_config('mueller')
    max_pages = store_config.get('max_pages', 20)
    items_per_page = store_config.get('items_per_page', 50)
    
    # Custom settings for MUELLER
    custom_settings = {
        **BaseFirecrawlSpider.custom_settings,
        'DOWNLOAD_DELAY': 1.5,  # MUELLER rate limiting
    ...
    def _get_wait_actions(self) -> list:
        """Get wait actions specific to MUELLER."""
        return [
            {
                "type": "wait",
    ...
    async def _extract_items_from_firecrawl_result(
        ...
        """Extract discount items from Firecrawl result with MUELLER-specific logic."""
        items = []
        ...
        try:
            ...
            if 'data' in result and isinstance(result['data'], dict):
                ...
                # Handle MUELLER's product structure
                if 'products' in structured_data:
                    # Multiple products in array
                    for product_data in structured_data['products']:
                        item = self._create_mueller_item_from_data(product_data, source_url, store_name)
                        if item:
                            items.append(item)
                
                elif 'product' in structured_data:
                    # Single product
                    item = self._create_mueller_item_from_data(structured_data['product'], source_url, store_name)
                    if item:
                        items.append(item)
                
                else:
                    # Try to extract from general data structure
                    item = self._create_mueller_item_from_data(structured_data, source_url, store_name)
                    if item:
                        items.append(item)
    ...
    def _create_mueller_item_from_data(
        self, 
        data: Dict[str, Any], 
        source_url: str, 
        store_name: str
    ) -> Optional[DiscountItem]:
        """Create a DiscountItem from MUELLER data with specific field mapping."""
        try:
            item = DiscountItem()
            ...
            # MUELLER-specific field mapping
            field_mapping = {
            ...
            if not item.get('discount_percentage') and item.get('original_price') and item.get('sale_price'):
                try:
                    original = self._parse_mueller_price(item['original_price'])
                    sale = self._parse_mueller_price(item['sale_price'])
            ...
            item = self._clean_mueller_item(item)
            
            return item if self._validate_mueller_item(item) else None
            
        except Exception as e:
            self.logger.error(f"Error creating MUELLER item from data: {e}")
            return None
    
    def _parse_mueller_price(self, price_str: str) -> Optional[float]:
        """Parse MUELLER price format."""
        if not price_str:
            return None
    ...
    def _clean_mueller_item(self, item: DiscountItem) -> DiscountItem:
        """Clean and normalize MUELLER item data."""
        # Clean price fields
        ...
        if item.get('url') and not item['url'].startswith('http'):
            if item['url'].startswith('/'):
                item['url'] = f"https://www.mueller.at{item['url']}"
            else:
                item['url'] = f"https://www.mueller.at/{item['url']}"
        
        return item
    
    def _validate_mueller_item(self, item: DiscountItem) -> bool:
        """Validate if a MUELLER item has required fields."""
        # MUELLER items should have at least a name or sale price
        return bool(
            item.get('name') or 
    ...
    def _get_next_page_url(self, current_url: str, next_page_num: int) -> Optional[str]:
        """Generate next page URL for MUELLER pagination."""
        try:
