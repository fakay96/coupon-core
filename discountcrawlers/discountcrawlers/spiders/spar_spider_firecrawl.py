"""
SPAR Spider using Firecrawl
=============================

This spider scrapes discount products from SPAR using Firecrawl API
...
class SparFirecrawlSpider(BaseFirecrawlSpider):
    """SPAR spider using Firecrawl for enhanced scraping."""
    
    name = "spar_spider_firecrawl"
    allowed_domains = ['interspar.at', 'www.interspar.at']
    start_urls = ['https://www.interspar.at/shop/lebensmittel/search/?query=*&q=*&hitsPerPage=80&page=1&filter=is-on-promotion:true&substringFilter=pos-visible:8757~~~8958', 'https://www.interspar.at/shop/lebensmittel/search/?filter=is-on-promotion:true&hitsPerPage=80&page=1']
    ...
    custom_settings = {
        **BaseFirecrawlSpider.custom_settings,
        'DOWNLOAD_DELAY': 1.5,  # SPAR rate limiting
    ...
    def _clean_spar_item(self, item: DiscountItem) -> DiscountItem:
        ...
        if item.get('url') and not item['url'].startswith('http'):
            if item['url'].startswith('/'):
                item['url'] = f"https://www.spar.at{item['url']}"
            else:
                item['url'] = f"https://www.spar.at/{item['url']}"
