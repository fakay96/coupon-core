#!/usr/bin/env python3
"""
Test script for Firecrawl Spider
================================

This script tests the Firecrawl spider integration to ensure it works correctly.
"""

import asyncio
import os
import sys
from datetime import datetime

# Add the discountcrawlers directory to the path
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'discountcrawlers'))

from discountcrawlers.firecrawl_integration import (
    FirecrawlConfig, 
    FirecrawlClient, 
    FirecrawlRequest,
    FirecrawlDiscountExtractor
)
from discountcrawlers.items import DiscountItem

# Load API key from environment
API_KEY = os.getenv('FIRECRAWL_API_KEY')
if not API_KEY:
    print("❌ FIRECRAWL_API_KEY environment variable not set")
    print("Please set your Firecrawl API key:")
    print("export FIRECRAWL_API_KEY='fc-your-api-key-here'")
    sys.exit(1)

async def test_firecrawl_extractor():
    """Test the Firecrawl discount extractor."""
    print("🔍 Testing Firecrawl Discount Extractor...")
    
    config = FirecrawlConfig(
        api_key=API_KEY,
        timeout=60,
        max_retries=3,
        max_concurrent_requests=2  # Reduced for rate limits
    )
    
    async with FirecrawlClient(config) as client:
        extractor = FirecrawlDiscountExtractor(client)
        
        # Test with a simple page
        test_url = "https://httpbin.org/html"
        store_name = "test_store"
        
        try:
            item = await extractor.extract_discount_data(test_url, store_name)
            
            if item:
                print("✅ Discount extraction successful!")
                print(f"   - URL: {item.get('url', 'N/A')}")
                print(f"   - Source: {item.get('source', 'N/A')}")
                print(f"   - Name: {item.get('name', 'N/A')}")
                print(f"   - Title: {item.get('title', 'N/A')}")
                print(f"   - Description: {item.get('description', 'N/A')[:100]}...")
                print(f"   - Price: {item.get('price', 'N/A')}")
                print(f"   - Original Price: {item.get('original_price', 'N/A')}")
                print(f"   - Discount: {item.get('discount_percentage', 'N/A')}%")
                print(f"   - Brand: {item.get('brand', 'N/A')}")
                print(f"   - Stock: {item.get('stock_info', 'N/A')}")
                print(f"   - Images: {len(item.get('image_urls', []))}")
                print(f"   - Crawled: {item.get('crawled_at', 'N/A')}")
                return True
            else:
                print("❌ Discount extraction returned no data")
                return False
                
        except Exception as e:
            print(f"❌ Discount extraction error: {e}")
            return False

async def test_firecrawl_client():
    """Test the Firecrawl client directly."""
    print("\n🔧 Testing Firecrawl Client...")
    
    config = FirecrawlConfig(
        api_key=API_KEY,
        timeout=60,
        max_retries=3,
        max_concurrent_requests=2
    )
    
    async with FirecrawlClient(config) as client:
        # Test basic scraping
        request = FirecrawlRequest(
            url="https://httpbin.org/html",
            formats=["markdown", "html"],
            only_main_content=True
        )
        
        try:
            result = await client.scrape_url(request)
            
            if result.get("success"):
                print("✅ Client scraping successful!")
                data = result.get("data", {})
                print(f"   - Markdown length: {len(data.get('markdown', ''))} chars")
                print(f"   - HTML length: {len(data.get('html', ''))} chars")
                print(f"   - Metadata: {data.get('metadata', {}).get('sourceURL', 'N/A')}")
                return True
            else:
                print(f"❌ Client scraping failed: {result.get('error', 'Unknown error')}")
                return False
                
        except Exception as e:
            print(f"❌ Client scraping error: {e}")
            return False

async def test_crawling():
    """Test crawling functionality."""
    print("\n🕷️ Testing Crawling...")
    
    config = FirecrawlConfig(
        api_key=API_KEY,
        timeout=60,
        max_retries=3,
        max_concurrent_requests=2
    )
    
    async with FirecrawlClient(config) as client:
        try:
            # Start a small crawl job
            crawl_result = await client.crawl_url(
                url="https://httpbin.org",
                limit=2,
                scrape_options={
                    "formats": ["markdown"],
                    "onlyMainContent": True
                }
            )
            
            if crawl_result.get("success"):
                crawl_id = crawl_result.get("id")
                print(f"✅ Crawl job started successfully! ID: {crawl_id}")
                
                # Check status
                status_result = await client.check_crawl_status(crawl_id)
                
                if status_result.get("status"):
                    print(f"   - Crawl status: {status_result.get('status')}")
                    print(f"   - Total pages: {status_result.get('total', 'N/A')}")
                    print(f"   - Completed: {status_result.get('completed', 'N/A')}")
                    return True
                else:
                    print(f"❌ Status check failed: {status_result.get('error', 'Unknown error')}")
                    return False
            else:
                print(f"❌ Crawl job failed: {crawl_result.get('error', 'Unknown error')}")
                return False
                
        except Exception as e:
            print(f"❌ Crawling test error: {e}")
            return False

async def test_rate_limiting():
    """Test rate limiting handling."""
    print("\n⏱️ Testing Rate Limiting...")
    
    config = FirecrawlConfig(
        api_key=API_KEY,
        timeout=30,
        max_retries=2,
        max_concurrent_requests=1
    )
    
    async with FirecrawlClient(config) as client:
        # Make multiple requests quickly to test rate limiting
        requests = []
        for i in range(3):
            request = FirecrawlRequest(
                url=f"https://httpbin.org/html?test={i}",
                formats=["markdown"]
            )
            requests.append(client.scrape_url(request))
        
        try:
            results = await asyncio.gather(*requests, return_exceptions=True)
            
            successful = 0
            rate_limited = 0
            
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    if "rate limit" in str(result).lower():
                        rate_limited += 1
                        print(f"   - Request {i+1}: Rate limited (expected)")
                    else:
                        print(f"   - Request {i+1}: Error - {result}")
                elif result.get("success"):
                    successful += 1
                    print(f"   - Request {i+1}: Success")
                else:
                    print(f"   - Request {i+1}: Failed - {result.get('error', 'Unknown')}")
            
            print(f"   - Successful: {successful}")
            print(f"   - Rate limited: {rate_limited}")
            
            # Consider it successful if we got at least one success or proper rate limiting
            return successful > 0 or rate_limited > 0
                
        except Exception as e:
            print(f"❌ Rate limiting test error: {e}")
            return False

async def main():
    """Run all tests."""
    print("🚀 Firecrawl Spider Integration Tests")
    print("=" * 50)
    print(f"API Key: {API_KEY[:10]}...{API_KEY[-4:] if len(API_KEY) > 14 else '***'}")
    print(f"Timestamp: {datetime.now().isoformat()}")
    
    tests = [
        ("Firecrawl Client", test_firecrawl_client),
        ("Discount Extractor", test_firecrawl_extractor),
        ("Crawling", test_crawling),
        ("Rate Limiting", test_rate_limiting),
    ]
    
    results = []
    
    for test_name, test_func in tests:
        try:
            success = await test_func()
            results.append((test_name, success))
        except Exception as e:
            print(f"❌ {test_name} test crashed: {e}")
            results.append((test_name, False))
    
    # Summary
    print("\n" + "=" * 50)
    print("📊 Test Results Summary")
    print("=" * 50)
    
    passed = 0
    total = len(results)
    
    for test_name, success in results:
        status = "✅ PASS" if success else "❌ FAIL"
        print(f"{status} {test_name}")
        if success:
            passed += 1
    
    print(f"\nOverall: {passed}/{total} tests passed")
    
    if passed >= total - 1:  # Allow one failure for rate limiting
        print("🎉 Firecrawl spider integration is working correctly!")
        print("\n📝 Next Steps:")
        print("1. Update your spider URLs to target actual discount sites")
        print("2. Configure store-specific extraction rules")
        print("3. Run the spider with: scrapy crawl firecrawl")
        return 0
    else:
        print("⚠️  Some tests failed. Please check the implementation.")
        return 1

if __name__ == "__main__":
    try:
        exit_code = asyncio.run(main())
        sys.exit(exit_code)
    except KeyboardInterrupt:
        print("\n⏹️  Tests interrupted by user")
        sys.exit(1)
    except Exception as e:
        print(f"\n💥 Unexpected error: {e}")
        sys.exit(1) 